...
<html class="no-js" lang="en">
  ...
  <!-- Off Canvas Menu -->
          <div id="offCanvasLeft" class="off-canvas position-left hide-for-large" data-off-canvas data-position="left">
			...
			<li class="active"><a href="Home_News.php">News & Highlights</a></li>
...
<!-- main -->
	...
	<div class="row">
		...
		$PerPage = 5;
		 ...
		 $stmt = $conn-> query("SELECT COUNT(*) FROM news");
		 ...
		 //QUERY TO FETCH BENEFICIARIES
		   $stmt = $conn->query("SELECT * FROM news 
					ORDER BY news_date_uploaded ASC
		...
		echo '<div class="media-object" style="border: 2px;">
			...
			<div class="media-object-section">
				<div class="thumbnail">
					 <img src="data:image/jpeg;base64,'.base64_encode( $row["image_data"] ).'" width="250" height="250"></img>
				</div>
			</div>
			<div class="media-object-section main-section" style="text-align: justify;">
				<h4>'.strtoupper($row["news_title"]).'</h4>
				<small>Author: '.$row["news_author"].' | Posted on: '.$newDate = date('F d, Y', strtotime($row["news_date_uploaded"])).'</small>
				<br>
