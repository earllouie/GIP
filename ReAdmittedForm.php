                ...
                <nav class="top-bar-left">
                  <ul class="dropdown menu" data-dropdown-menu>
					...
					<li class="active">
						<a href="#">Update Beneficiaries</a>
							<ul class="vertical menu">
								<li class="active"><a href="ReAdmittedForm.php">Re-admitted</a></li>
								<li><a href="AbsorbedForm.php">Absorb</a></li>
							</ul>
					</li>
...
<!-- main -->
    <div id="main-content">
		...
		<div class="row" >
			<div class="large-3 columns">
				<form action="ReAdmittedForm.php" method="POST" >
				<label>Choose Form to Update:</label>
				<select name="Reason_For_Updating">
					<option value="RE-ADMITTED">RE-ADMITTED</option>
					<option value="ABSROB">ABSROB</option>
				</select>
			</div>
		
			<div class="large-3 columns">
				<label>Search by:</label>
					<select id="Applicant_Status_Of_Contract" name="Filter_by">
						<option value="First_Name">FIRST NAME</option>
						<option value="Middle_Name">MIDDLE NAME</option>
						<option value="Last_Name">LAST NAME</option>
						<option value="GIP_ID">GIP ID</option>
						<option value="BReN">BIRTH REFERENCE NUMBER</option>
					</select>
			</div>
			
			<div class="large-4 columns">	
					<label>Search for:	</label>
					<input type="text" name="Look_for" autocomplete="off" ></input>
			</div>
			
			<div class="large-2 columns">	
					</br>
				<input type="submit" class="button small" name="search_profile" value="SEARCH">
			</div>
			</form>
		...
		<div class="large-12 columns">
			<?php
				...
				{	
						...
						if($_SESSION['userlevel']=="Encoder"){
						$stmt = $conn->query("SELECT * FROM `beneficiary` WHERE `". $_POST["Filter_by"]. "` = '". $_POST["Look_for"] ."' AND Region = '".$_SESSION['region']."' ");
						}else if ($_SESSION['userlevel'] =="Admin"){
							$stmt = $conn->query("SELECT * FROM `beneficiary` WHERE `". $_POST["Filter_by"]. "` = '". $_POST["Look_for"] ."'");
						}
						echo "
						...
						while($row = $stmt->fetch())
							{	
								...
								if($row>0){
									...
									echo  "<tr>	
												<input type='hidden' name='action' value=".$_POST['Reason_For_Updating'].">
												<input type='hidden' name='ID' value=".$row[1].">
												<td><input type='submit' class='button' value='UPDATE'></input></td>
												</form>
												<td>".$row[0]."</td>
												<td>".$_POST['ID'] = $row[4]."</td>
									...
									echo "<tr><td>No matching record found!</td></tr>";
								}
	...
	<div class="row">
		<div class="reveal" id="exampleModal1" data-reveal>
			<h3 class="text-center">Are you sure you want to log out?</h3><br>
